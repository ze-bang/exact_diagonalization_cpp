...
def plot_connectivity_by_site(vertices, edges, edges_2nn, edges_3nn, output_dir, cluster_name, 
    ...
    """
    Plot NN, 2NN, 3NN connectivity for each sublattice site to visually verify connections.
    
    Creates two plots:
    1. connectivity_by_site.png: 3x3 grid showing Sites 0,1,2 with their NN/2NN/3NN neighbors
    2. all_bonds_overview.png: Overview of all bonds for each neighbor type
    
    Args:
        vertices: Dictionary of {vertex_id: (x, y)}
        edges: List of NN edges
        edges_2nn: List of 2NN edges
        edges_3nn: List of 3NN edges
        output_dir: Directory to save plots
        cluster_name: Name for the cluster
        vertex_to_cell: Map vertex_id to (i, j, site_idx)
        dim1, dim2: Lattice dimensions
    ...
    try:
        ...
        sublattices = {v: get_sublattice_index(v, vertex_to_cell) for v in vertices}
        ...
        # Plot 1: Connectivity by site (3x3 grid)
        fig, axes = plt.subplots(3, 3, figsize=(15, 15))
        fig.suptitle(f'Kagome Lattice Connectivity by Site ({dim1}x{dim2} {"PBC" if use_pbc else "OBC"})', 
                     fontsize=16, y=1.02)
        
        for row, center_site in enumerate([0, 1, 2]):
            center_pos = np.array(vertices[center_site])
            center_sub = sublattices[center_site]
            ...
            for col, (neighbor_type, neighbor_list, bond_label) in enumerate([
                ...
                for n in neighbors:
                    ...
                    ax.scatter(n_pos[0], n_pos[1], s=200, c=sublattice_colors[sublattices[n]], 
                ...
                ax.set_title(f'Site {center_site} (Sub {center_sub}): {bond_label} ({len(neighbors)} nbrs)', 
                            fontsize=10)
                ax.set_aspect('equal')
