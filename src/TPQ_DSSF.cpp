...
int main(int argc, char* argv[]) {
    ...
    for (int i = rank; i < num_files; i += size) {
        ...
        if (method == "krylov") {
            ...
            // Build transverse (global) operators using the two transverse unit vectors e1 and e2
            for (size_t q_idx = 0; q_idx < momentum_points.size(); ++q_idx) {
                const auto &Q = momentum_points[q_idx];
                // Extract transverse basis vectors for this momentum
                const auto &b1 = transverse_basis_1[q_idx];
                const auto &b2 = transverse_basis_2[q_idx];
                ...
                for (size_t combo_idx = 0; combo_idx < spin_combinations.size(); ++combo_idx) {
                    ...
                    // e1 component
                    {
                        ...
                        try {
                            ...
                            observable_names.push_back(name_ss.str());
                            observables_1.push_back(std::move(op1));
                            observables_2.push_back(std::move(op2));
                        } catch (const std::exception &e) {
                            if (rank == 0) {
                                std::cerr << "Failed to build TransverseOperator (e1) for q_idx=" 
                                          << q_idx << ": " << e.what() << std::endl;
                            }
                        }
                    }
                    // e2 component
                    {
                        ...
                        try {
                            ...
                            if (rank == 0) {
                                std::cerr << "Failed to build TransverseOperator (e2) for q_idx=" 
                                          << q_idx << ": " << e.what() << std::endl;
                            }
                        }
                    }
            ...
            if (observables_1.empty() && observables_2.empty()) {
                ...
                std::string taylor_dir = output_dir + "/global";
                ensureDirectoryExists(taylor_dir);
                computeObservableDynamics_U_t(
                    U_t,
                    tpq_state,
