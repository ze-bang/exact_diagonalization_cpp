...
void GPUEDWrapper::runGPUMicrocanonicalTPQCheckpoint(void* gpu_op_handle,
                                                    int num_samples,
                                                    const std::vector<double>& checkpoint_betas,
                                                    std::vector<double>& eigenvalues,
                                                    std::string dir,
                                                    double large_value) {
    if (!gpu_op_handle) {
    ...
    GPUOperator* gpu_op = static_cast<GPUOperator*>(gpu_op_handle);
    int hilbert_dim = gpu_op->getDimension();
    
    std::cout << "Running GPU Microcanonical TPQ with checkpoint mode (dim=" 
              << hilbert_dim << ", checkpoints=" << checkpoint_betas.size() << ")\n";
    
    GPUTPQSolver tpq_solver(gpu_op, hilbert_dim);
    tpq_solver.runMicrocanonicalTPQCheckpoint(num_samples, checkpoint_betas, eigenvalues, dir, large_value);
    
    std::cout << "\nGPU Microcanonical TPQ Checkpoint complete\n";
}

void GPUEDWrapper::runGPUMicrocanonicalTPQFixedSzCheckpoint(void* gpu_op_handle,
                                                           int n_up,
                                                           int num_samples,
                                                           const std::vector<double>& checkpoint_betas,
                                                           std::vector<double>& eigenvalues,
    ...
    std::cout << "Running GPU Microcanonical TPQ Fixed Sz with checkpoint mode (N_up=" << n_up 
              << ", dim=" << fixed_sz_dim << ", checkpoints=" << checkpoint_betas.size() << ")\n";
    ...
    std::cout << "\nGPU Microcanonical TPQ Fixed Sz Checkpoint complete\n";
}

void GPUEDWrapper::runGPUCanonicalTPQCheckpoint(void* gpu_op_handle,
                                               int num_samples,
                                               const std::vector<double>& checkpoint_betas,
                                               std::vector<double>& energies,
                                               std::string dir,
    ...
    std::cout << "Running GPU Canonical TPQ with checkpoint mode (dim=" 
              << hilbert_dim << ", checkpoints=" << checkpoint_betas.size() << ")\n";
    
    GPUTPQSolver tpq_solver(gpu_op, hilbert_dim);
    tpq_solver.runCanonicalTPQCheckpoint(num_samples, checkpoint_betas, energies, dir, delta_beta, taylor_order);
    
    std::cout << "\nGPU Canonical TPQ Checkpoint complete\n";
}

void GPUEDWrapper::runGPUCanonicalTPQFixedSzCheckpoint(void* gpu_op_handle,
                                                      int n_up,
                                                      int num_samples,
                                                      const std::vector<double>& checkpoint_betas,
                                                      std::vector<double>& energies,
    ...
    std::cout << "Running GPU Canonical TPQ Fixed Sz with checkpoint mode (N_up=" << n_up 
              << ", dim=" << fixed_sz_dim << ", checkpoints=" << checkpoint_betas.size() << ")\n";
    ...
    std::cout << "\nGPU Canonical TPQ Fixed Sz Checkpoint complete\n";
}
